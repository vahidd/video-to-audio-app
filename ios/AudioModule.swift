...
class AudioModule: RCTEventEmitter {
  ...
  func extractAudio(
    ...
    getVideoDuration(filePath: inputPath) { durationSeconds in
  ...
  func getDuration(
    ...
    let escapedFilePath = inputPath.replacingOccurrences(of: "\"", with: "\\\"")
    let command = "-v error -select_streams a:0 -show_entries stream=duration -of default=noprint_wrappers=1:nokey=1 \"\(escapedFilePath)\""
    ...
    FFprobeKit.executeAsync(command) { session in
      ...
      if trimmedOutput.isEmpty {
        reject("NO_AUDIO_TRACK", "No audio stream found in file", nil)
        return
      }
      
      if let duration = Double(trimmedOutput) {
        resolve(duration)
      } else {
        reject("PARSE_ERROR", "Could not parse duration from FFprobe output", nil)
      }
    }
  }
  ...
  private func getVideoDuration(filePath: String, completion: @escaping (Double?) -> Void) {
    let escapedFilePath = filePath.replacingOccurrences(of: "\"", with: "\\\"")
    let command = "-v error -select_streams v:0 -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 \"\(escapedFilePath)\""
    FFprobeKit.executeAsync(command) { session in
      let output = session?.getOutput() ?? ""
